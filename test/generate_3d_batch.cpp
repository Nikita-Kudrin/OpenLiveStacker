...
int main(int argc, char** argv) {
    ...
    if (!fs::exists(data_dir)) {
        ...
        if (!fs::exists(data_dir)) {
            std::cerr << "Data directory does not exist: test/data/original_for_3d/ or /opt/GitHub/OpenLiveStacker/test/data/original_for_3d/" << std::endl;
            return 1;
        }
    }
    ...
    for (const auto& entry : fs::directory_iterator(data_dir)) {
        if (entry.is_regular_file()) {
            ...
            if (ext_lower == ".png" || ext_lower == ".jpg" || ext_lower == ".jpeg" || ext_lower == ".tiff" || ext_lower == ".tif") {
