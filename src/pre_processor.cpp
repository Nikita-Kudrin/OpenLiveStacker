    ...
    public:
        ...
        void darks_and_flats(cv::Mat &frame)
        {
            ...
            for(;i<limit;i+=4,p+=4,d+=4,f+=4) {
                cv::v_float32x4 v=cv::v_max(zero,cv::v_sub(cv::v_load(p), cv::v_load(d)));
                v = cv::v_min(cv::v_mul(cv::v_load(f), v),one);
                cv::v_store(p,v);
            }
#endif            
            for(;i<N;i++) {
        ...
        void darks_only(cv::Mat &frame)
        {
            ...
            for(;i<limit;i+=4,p+=4,d+=4) {
                auto v=cv::v_max(zero,cv::v_sub(cv::v_load(p), cv::v_load(d)));
                cv::v_store(p,v);
            }
#endif            
            for(;i<N;i++) {
        ...
        void remove_gradient_mono(cv::Mat &img)
        {
            ...
            for(int r = 0;r<rows;r++) {
                ...
                for(;c<limit;c+=4,p+=4) {
                    ...
                    Ay_v = cv::v_add(Ay_v, cv::v_mul(val, vec_y));
                    Ax_v = cv::v_add(Ax_v, cv::v_mul(val, vec_x));
                }
            ...
            for(int r = 0;r<rows;r++) {
                ...
                for(;c<limit;c+=4,p+=4) {
                    cv::v_float32x4 plane_v = cv::v_add(cv::v_mul(cv::v_load(xv + c), Ax_v), row_p_v);
                    cv::v_float32x4 val_v = cv::v_load(p);
                    val_v = cv::v_max(v_0,cv::v_min(v_1,cv::v_sub(val_v, plane_v)));
                    cv::v_store(p,val_v);
                }
                #endif
                for(;c<cols;c++) {
        ...
        void remove_gradient_color(cv::Mat &img)
        {
            ...
            for(int r = 0;r<rows;r++) {
                ...
                for(;c<limit;c+=4,p+=12) {
                    ...
                    ARy_v = cv::v_add(ARy_v, cv::v_mul(valR, vec_y));
                    ARx_v = cv::v_add(ARx_v, cv::v_mul(valR, vec_x));

                    AGy_v = cv::v_add(AGy_v, cv::v_mul(valG, vec_y));
                    AGx_v = cv::v_add(AGx_v, cv::v_mul(valG, vec_x));

                    ABy_v = cv::v_add(ABy_v, cv::v_mul(valB, vec_y));
                    ABx_v = cv::v_add(ABx_v, cv::v_mul(valB, vec_x));
                }
            ...
            for(int r = 0;r<rows;r++) {
                ...
                for(;c<limit;c+=4,p+=12) {
                    cv::v_float32x4 planeR_v = cv::v_add(cv::v_mul(cv::v_load(xv + c), ARx_v), rowR_p_v);
                    cv::v_float32x4 planeG_v = cv::v_add(cv::v_mul(cv::v_load(xv + c), AGx_v), rowG_p_v);
                    cv::v_float32x4 planeB_v = cv::v_add(cv::v_mul(cv::v_load(xv + c), ABx_v), rowB_p_v);
                    cv::v_float32x4 valR_v,valG_v,valB_v;
                    cv::v_load_deinterleave(p,valR_v,valG_v,valB_v);
                    valR_v = cv::v_max(v_0,cv::v_min(v_1,cv::v_sub(valR_v, planeR_v)));
                    valG_v = cv::v_max(v_0,cv::v_min(v_1,cv::v_sub(valG_v, planeG_v)));
                    valB_v = cv::v_max(v_0,cv::v_min(v_1,cv::v_sub(valB_v, planeB_v)));
